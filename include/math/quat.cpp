...
quat operator*(const quat& q1, const quat& q2) {
	return quat(
		q2.x * q1.w + q2.y * q1.z + q2.w * q1.x - q2.z * q1.y,
		q2.y * q1.w + q2.z * q1.x + q2.w * q1.y - q2.x * q1.z,
		q2.x * q1.y + q2.z * q1.w + q2.w * q1.z - q2.y * q1.x,
		q2.w * q1.w - q2.x * q1.x - q2.y * q1.y - q2.z * q1.z
	);
}

bool operator==(const quat& left, const quat& right) {
	return (fabsf(left.x - right.x) <= kEpsilon &&
		fabsf(left.y - right.y) <= kEpsilon &&
		fabsf(left.z - right.z) <= kEpsilon &&
		fabsf(left.w - right.w) <= kEpsilon);
}
...
bool sameOrientation(const quat& l, const quat& r) {
	return (fabsf(l.x - r.x) <= kEpsilon &&
		fabsf(l.y - r.y) <= kEpsilon &&
		fabsf(l.z - r.z) <= kEpsilon &&
		fabsf(l.w - r.w) <= kEpsilon) ||
		(fabsf(l.x + r.x) <= kEpsilon &&
			fabsf(l.y + r.y) <= kEpsilon &&
			fabsf(l.z + r.z) <= kEpsilon &&
			fabsf(l.w + r.w) <= kEpsilon);
}
...
quat mix(const quat& from, const quat& to, float t) { // lerp approximation
...
quat operator^(const quat& q, float f) {
	float angle = 2.0f * acos(q.scalar);
...
quat slerp(const quat& start, const quat& end, float t) {
	if (fabsf(dot(start, end)) > 1.0f - kEpsilon) {
...
quat mat4ToQuat(const mat4& m) {
	...
	vec3 right = cross(forward, right);
