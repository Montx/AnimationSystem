...
void SampleTracks::Render(float inAspectRatio) {
	mat4 model;
...
ScalarFrame SampleTracks::MakeFrame(float time, float value) {
	return MakeFrame(time, 0.0f, value, 0.0f);
}

VectorFrame SampleTracks::MakeFrame(float time, const vec3& value) {
	return MakeFrame(time, vec3(), value, vec3());
}

QuaternionFrame SampleTracks::MakeFrame(float time, const quat& value) {
	return MakeFrame(time, quat(0, 0, 0, 0), value, quat(0, 0, 0, 0));
}

ScalarFrame SampleTracks::MakeFrame(float time, float in, float value, float out) {
	ScalarFrame result;
	result.mTime = time;
	result.mIn[0] = in;
...
VectorFrame SampleTracks::MakeFrame(float time, const vec3& in, const vec3& value, const vec3& out) {
	VectorFrame result;
	result.mTime = time;
	result.mIn[0] = in.x;
	result.mIn[1] = in.y;
...
QuaternionFrame SampleTracks::MakeFrame(float time, const quat& in, const quat& out, const quat& value) {
...
ScalarTrack SampleTracks::MakeScalarTrack(Interpolation interp, int numFrames, ...) {
	ScalarTrack result;
...
VectorTrack SampleTracks::MakeVectorTrack(Interpolation interp, int numFrames, ...) {
	VectorTrack result;
	result.SetInterpolation(interp);
	result.Resize(numFrames);
...
QuaternionTrack SampleTracks::MakeQuaternionTrack(Interpolation interp, int numFrames, ...) {
