...
Pose LoadBindPose(cgltf_data* handle) {
	Pose restPose = LoadRestPose(handle);
	...
	unsigned int numSkins = handle->skins_count;
	for (unsigned int i = 0; i < numSkins; ++i) {
		cgltf_skin* skin = &(handle->skins[i]);
		...
		for (int j = 0; j < numJoints; ++j) {
			...
			int jointIndex = GLTFHelpers::GetNodeIndex(jointNode, handle->nodes, numBones);
